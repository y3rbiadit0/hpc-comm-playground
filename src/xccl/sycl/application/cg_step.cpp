...
// CG iteration communication skeleton (see src/mpi/cuda/application/cg_step.cu): SpMV halo
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    int all_sides_ok = 1;  // must outlive the oneCCL scope below
    ...
    // uct_mm_ep_flush. This scope closes them first.
    {
      ...
      for (const std::size_t side : sides) {
         ...
         * The queue is in-order and every submission and oneCCL event is waited
         * on, so composing them is exactly the step this benchmark has always
         * timed; only the phase pass adds synchronization. */
        const auto pack = [&]() {
          if (local_cols > 0) {
        ...
        const auto reduce = [&]() {
          ...
          ccl::allreduce(partial_qq, result_qq, 1, ccl::datatype::float64, ccl::reduction::sum, comm, stream).wait();
        };
        ...
        if (rank == 0) {
          ...
          report.name = "sycl_oneccl_cg_step";
          report.n = side;
          report.ranks = ranks;
